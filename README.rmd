...
USDZAR_sd <- USDZAR %>% mutate(YearMonth=format(date, "%Y%B")) %>%
...
Hi_Vol <- USDZAR_sd %>% filter(SD>TopQtile) %>% pull(YearMonth) # high volatility periods
Lo_Vol <- USDZAR_sd %>% filter(SD<BotQtile) %>% pull(YearMonth) # low volatility periods
